...
- The documentation site's hero banner is the warm end of the brand — gold running to orange — instead of the navy it shared with every other surface. Cream lettering could not come with it — it manages 1.6:1 on gold — so the banner carries a darkened navy ink measuring 7.2:1 against its darkest stop, and the call to action takes the navy the background gave up. The pinwheel weave over the top lightens only, never tints dark, so that measured floor is a floor and not an average. The warm ramp and that ink are tokens in `custom.css`, shared with the [patb-cli](https://github.com/thiagocolen/patb-cli) docs, which take the hotter slice of the same span.
