...
Retrieval is keyword overlap against `src/storage/vector-store.json`, so an unusual
phrasing can match nothing. Rephrase using terms closer to the stored material (e.g.
"Conway", "IELTS", "EC2"). If the store is missing or empty, run `npm run ingest` to
regenerate it.
