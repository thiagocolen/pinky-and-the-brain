...
| --- | --- |
| `OPENAI_API_KEY` or `GOOGLE_API_KEY` | At least one LLM provider key is required. `src/utils/model.ts` prefers Google Gemini when `GOOGLE_API_KEY` is set, otherwise falls back to OpenAI. |
...
`validateConfig()` throws at startup if neither LLM key nor `PATBA_API_KEY` is
...
| --- | --- | --- |
| `GEMINI_MODEL` | `gemini-2.5-flash` | Gemini model name used when `GOOGLE_API_KEY` is set. |
