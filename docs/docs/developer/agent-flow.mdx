...
flowchart TD
    ...
    Compose --> Save["save_article<br/>→ ./articles/*.md<br/>deck, callouts and figures included"]
    ...
    Ask --> Publish["publish_article<br/>→ renders the layout, generates<br/>cover + figures, drafts via the blog's MCP server"]
...
5. **Save and iterate.** `save_article` writes `./articles/<slug>.md`; revisions go through
   `update_article`, only after Pinky confirms each change.
6. **Deliver.** Step 4c offers the blog, a folder, or neither.
